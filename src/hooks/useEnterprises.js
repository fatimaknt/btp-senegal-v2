...
export function useEnterprises() {
    ...
    const fetchEnterprises = async (filters, page = 1, limit = 20) => {
        setLoading(true);
        setError(null);
        try {
            let query = supabase
    ...
    const searchEnterprises = async (query, filters) => {
        setLoading(true);
        setError(null);
        try {
            ...
            const mockResults = MOCK_ENTERPRISES.filter(enterprise => enterprise.name.toLowerCase().includes(query.toLowerCase()) ||
    ...
    const getNearbyEnterprises = async (lat, lng, radius = 10000) => {
        try {
    ...
    const updateEnterprise = async (id, updates) => {
        try {
            const { data, error } = await supabase
                .from('enterprises')
                .update(updates)
