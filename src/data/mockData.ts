import type { Enterprise, Category, Review } from '../types/enterprise'
import { CATEGORIES } from './categories'

export const MOCK_CATEGORIES: Category[] = CATEGORIES.map(cat => ({
    id: cat.id,
    name: cat.name,
    icon: cat.icon,
    color: cat.color,
...
export function getEnterprisesByCategory(categoryId: string): Enterprise[] {
    return MOCK_ENTERPRISES.filter(enterprise => enterprise.category.id === categoryId)
}

export function getEnterprisesByCity(city: string): Enterprise[] {
    return MOCK_ENTERPRISES.filter(enterprise =>
        enterprise.city.toLowerCase().includes(city.toLowerCase())
    )
}

export function searchEnterprises(query: string): Enterprise[] {
    const searchTerm = query.toLowerCase()
    return MOCK_ENTERPRISES.filter(enterprise =>
        enterprise.name.toLowerCase().includes(searchTerm) ||
        enterprise.description.toLowerCase().includes(searchTerm) ||
        enterprise.services.some(service =>
